...
contract('RealitioProxy', () => {
  describe('binary', () => {
    it('should revert if the question has not been finalized', async () => {
      ...
      let failed = false
      try {
        await realitioProxy.resolveBinaryCondition('0x0')
      } catch (e) {
        failed = true
      }
    ...
    it('should succeed if the question has been finalized with yes', async () => {
      ...
      await realitioProxy.resolveBinaryCondition(questionId)
    ...
    it('should revert if the answer is neither 0 nor 1', async () => {
      ...
      try {
        ...
        failed = true
        assert(e.message.includes('Only binary answers are supported'))
      }
